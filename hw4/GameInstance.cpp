...
GameInstance::GameInstance(string path) {
    json_t *root;
    json_error_t jError;

    root = json_load_file(path.c_str(), 0, &jError);
    if (root == NULL) {
    ...
    try {
        definition = getJsonObject(root, "gamedef");
    ...
    json_t *state = NULL;
    try {
        state = getJsonObject(root, "gamestate");
    } catch (invalid_argument e) {
...
void GameInstance::save(const string path) {
    if (path.compare("") == 0) {
        throw invalid_argument("path is not a valid file name.");
    }
